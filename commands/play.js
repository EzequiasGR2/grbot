...
module.exports = {
  name: "play",
  description: "Vamos ouvir um pouco de música!!",
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (!name)
        return interaction
          .reply({
            content: `❌ Digite um nome de música válido.`,
            ephemeral: true,
          })
          .catch((e) => {});
      ...
      try {
        ...
        return interaction
          .editReply({ content: `❌ Sem resultados` })
          .catch((e) => {});
      }

      if (!res || !res.length || res.length < 1)
        return interaction
          .reply({ content: `❌ Nenhum resultado encontrado`, ephemeral: true })
          .catch((e) => {});
