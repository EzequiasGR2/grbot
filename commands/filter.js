...
module.exports = {
  ...
  run: async (client, interaction) => {
    try {
      ...
      .setDescription('** Explore o Ritmo, Escolha sua Magia Sonora Abaixo!**')
...
if(!filters?.includes(button?.customId)) return
      ...
      if (!filtre) return interaction?.editReply({ content: '❌ Nome inválido', ephemeral: true }).catch(e => { })
     filtre = filtre?.toLowerCase()
      ...
      if (filters?.includes(filtre?.toLowerCase())) {
        ...
        embed?.setDescription(`❌ Filtro não encontrado!!`.replace("{filters}", filters?.map(mr => `\`${mr}\``).join(", ")))
