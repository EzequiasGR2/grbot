...
module.exports = {
  name: "loop",
  description: "Ativa ou desativa o modo de repetição de música.",
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('**Repetindo! Deixe a música tocar indefinidamente. **')
      ...
      interaction?.reply({ embeds: [embed], components: [button], fetchReply: true }).then(async Message => {
        ...
        col.on('collect', async (button) => {
          ...
          if (!queue1 || !queue1.playing) {
            await interaction?.editReply({ content: '⚠️ Nenhuma música sendo reproduzida!!', ephemeral: true }).catch(e => { })
            await button?.deferUpdate().catch(e => {})
          }
          switch (button.customId) {
            case 'queue':
              const success = queue.setRepeatMode(2);
              interaction?.editReply({ content: `✅ Repetindo Fila!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'nowplaying':
              const success2 = queue.setRepeatMode(1);
              interaction?.editReply({ content: `✅ Repetição ativada!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'close':
              if (queue.repeatMode === 0) {
                await button?.deferUpdate().catch(e => {})
                return interaction?.editReply({ content: '⚠️ Repetição já desativada!!', ephemeral: true }).catch(e => { })
              }
              const success4 = queue.setRepeatMode(0);
              interaction?.editReply({ content: '▶️ Repetição desligada' }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
          }
        })
        col.on('end', async (button) => {
          ...
          const embed = new EmbedBuilder()
            .setColor('#fc5203')
            .setTitle('▶️ Repetição desligada!!')
            .setTimestamp()
