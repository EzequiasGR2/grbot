...
module.exports = {
  ...
  options: [{
    name: "number",
    description: "Digite quantas músicas você deseja pular",
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (number) {
        ...
        if (number <= 0) return interaction.reply({ content: '⚠️ Número inválido', ephemeral: true }).catch(e => { })

        try {
          let old = queue.songs[0];
          await client.player.jump(interaction, number).then(song => {
            return interaction.reply({ content: `⏯️ Pulou: **${old.name}**` }).catch(e => { })
          })
        } catch(e) {
        ...
        try {
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(success ? ` **PULOU** : **${old.name}**` : '❌ A fila está vazia!')
            .setTimestamp();
